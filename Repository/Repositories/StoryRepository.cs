...
namespace Repository.Repositories
{
    ...
    {
        public StoryRepository(AppDbContext db, ISnowflakeIdGenerator ids) : base(db, ids)
        {
        }

        public Task<author?> GetAuthorAsync(ulong accountId, CancellationToken ct = default)
            => _db.authors
                  .Include(a => a.rank)
                  .Include(a => a.account)
                  .FirstOrDefaultAsync(a => a.account_id == accountId, ct);

        public Task<List<tag>> GetTagsByIdsAsync(IEnumerable<uint> tagIds, CancellationToken ct = default)
        {
            var ids = tagIds.ToArray();
            return _db.tags.Where(t => ids.Contains(t.tag_id)).ToListAsync(ct);
        }

        public async Task<story> AddStoryAsync(story entity, IEnumerable<uint> tagIds, CancellationToken ct = default)
        {
            EnsureId<story, ulong>(entity, nameof(story.story_id), () => NewId());
        ...
        public Task<List<story>> GetStoriesByAuthorAsync(ulong authorId, CancellationToken ct = default)
            => _db.stories
                  .Include(s => s.story_tags).ThenInclude(st => st.tag)
                  .Where(s => s.author_id == authorId)
                  .ToListAsync(ct);

        public Task<story?> GetStoryWithDetailsAsync(ulong storyId, CancellationToken ct = default)
            => _db.stories
        ...
        public Task<story?> GetStoryForAuthorAsync(ulong storyId, ulong authorId, CancellationToken ct = default)
            => _db.stories
                  .Include(s => s.story_tags).ThenInclude(st => st.tag)
        ...
        public async Task ReplaceStoryTagsAsync(ulong storyId, IEnumerable<uint> tagIds, CancellationToken ct = default)
        {
        ...
        public Task<content_approve?> GetLatestContentApproveAsync(ulong storyId, string source, CancellationToken ct = default)
            => _db.content_approves
                  .Where(c => c.story_id == storyId && c.approve_type == "story" && c.source == source)
                  .OrderByDescending(c => c.created_at)
                  .FirstOrDefaultAsync(ct);

        public Task<List<content_approve>> GetContentApprovalsForStoryAsync(ulong storyId, CancellationToken ct = default)
            => _db.content_approves
